...
class poi_libro_mayor_wiz(models.TransientModel):
    ...
    grouping = fields.Selection([('account','Cuenta'), ('analytic',u'Cuenta-Analítica'), ('acc_partner','Cuenta-Empresa'), ('partner_acc','Empresa-Cuenta'), ('acc_seg', 'Cuenta-Segmento'), ('acc_bank', 'Cuenta-Cuenta Bancaria')], string=u"Agrupación", default='account', required=True)
    ...
    segment_ids = fields.Many2many('account.segment', 'poi_libro_mayor_wizard_segment_rel', 'wizard_id', 'segment_id',string=u'Segmentos', readonly=False)
    bank_account_ids = fields.Many2many('res.bank.account', 'poi_libro_mayor_wizard_bank_account_rel', 'wizard_id', 'bank_account_id', string=u'Cuentas Bancarias', readonly=False)
    ...
    def launch_report(self):
        ...
        if wizard.grouping == "analytic":
            ...
            report_view = 'poi.libro.mayor'
        if wizard.grouping == "acc_seg":
            context['search_default_group_account'] = 1
            context['search_default_group_segment'] = 1
            report_view = 'poi.libro.mayor'
        if wizard.grouping == "acc_bank":
            context['search_default_group_account'] = 1
            context['search_default_group_bank'] = 1
            report_view = 'poi.libro.mayor'
        ...
        if wizard.segment_ids:
            segments = []
            for row in wizard.segment_ids:
                segments.append(row.id)
            context['segment_ids'] = segments
        if wizard.bank_account_ids:
            banks = []
            for row in wizard.bank_account_ids:
                banks.append(row.id)
            context['bank_account_ids'] = banks
        ...
        action_report = {
            ...
            'target': 'current',
            'options': {'clear_breadcrumbs': True, 'disable_groupby': True},
            'flags': {'form': {'action_buttons': True, 'options': {'clear_breadcrumbs': True, 'disable_groupby': True}}}
        }
...
class poi_libro_mayor(models.Model):
    ...
    segment_id = fields.Many2one('account.segment', 'Segmento', readonly=True)
    bank_account_id = fields.Many2one('res.bank.account', 'Cuenta Bancaria', readonly=True)
    ...
    def init(self):
        ...
        cr = self._cr
        b=False
        self.build_view(b=b, cr=cr, context={})
        #View can be rebuilt from Wizard

    @api.model
    def build_view(self, b, cr, context=None):
        ...
        if context.get('segment_ids', False):
            where_part = "%s and aml.segment_id IN %s" % (where_part,str(context.get('segment_ids',[0])).replace("[","(").replace("]",")"))
            where_bal = "%s and aml.segment_id IN %s" % (where_bal,str(context.get('segment_ids',[0])).replace("[","(").replace("]",")"))
        if context.get('bank_account_ids', False):
            where_part = "%s and aml.bank_account_id IN %s" % (where_part,str(context.get('bank_account_ids',[0])).replace("[","(").replace("]",")"))
            where_bal = "%s and aml.bank_account_id IN %s" % (where_bal,str(context.get('bank_account_ids',[0])).replace("[","(").replace("]",")"))
        ...
        if context.get('grouping', False):
            ...
            elif grouping == 'acc_seg':
                partition_clause = "partition by urep.account_id,urep.segment_id"
                order_clause = "order by account_id,segment_id,date,id"
            elif grouping == 'acc_bank':
                partition_clause = "partition by urep.account_id,urep.bank_account_id"
                order_clause = "order by account_id,bank_account_id,date,id"


        balance_union = ""
        if context.get('balance', False):
            order_clause = order_clause.replace(',date',',case_order,date')
            if grouping not in ('analytic', 'acc_partner'):
                balance_union = """UNION
                               select (aml.account_id * -1) as id, NULL as move_id, aml.account_id as account_id,0 as case_order, NULL as date, NULL AS analytic_id, NULL as period_id, NULL as journal_id, NULL as partner_id, null::int as segment_id, null::int as bank_account_id, NULL as company_id, '' as state
                                    , 'INICIAL' as name, 'INICIAL' as ref,  case when sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) > 0 then sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) else 0 end AS debit, case when sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) < 0 then sum(COALESCE(aml.credit, 0.0) - COALESCE(aml.debit, 0.0)) else 0 end AS credit,'' AS contra_accounts
                               from account_move_line aml inner join account_move am on am.id=aml.move_id
                               where am.date < '%s' %s
                               group by aml.account_id
                                """ % (context.get('date_from','1900-01-01'), where_bal)
            elif grouping == 'acc_partner':
                balance_union = """UNION
                    select (row_number() over() * -1)::int as id, move_id, account_id, case_order, date, analytic_id, period_id, journal_id, partner_id, segment_id, bank_account_id, company_id, state, name, ref, debit, credit, contra_accounts from(
                                   select  NULL::int as move_id, aml.account_id::int as account_id,0 as case_order, NULL::date as date, NULL::int AS analytic_id, NULL::text as period_id,null::int as segment_id, null::int as bank_account_id, NULL::int as journal_id, aml.partner_id, NULL::int as company_id, ''::varchar as state
                                        , 'INICIAL'::varchar as name, 'INICIAL'::varchar as ref,  case when sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) > 0 then sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) else 0 end AS debit, case when sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) < 0 then sum(COALESCE(aml.credit, 0.0) - COALESCE(aml.debit, 0.0)) else 0 end AS credit,''::text AS contra_accounts
                                   from account_move_line aml inner join account_move am on am.id=aml.move_id
                                   where am.date < '%s' %s
                                   group by aml.account_id, aml.partner_id) as bha

                    """ % (context.get('date_from','1900-01-01'), where_bal)
            elif grouping == 'analytic':
                balance_union = """UNION
                    select (row_number() over() * -1)::int as id, move_id, account_id, case_order, date, analytic_id, period_id, journal_id, partner_id, segment_id, bank_account_id, company_id, state, name, ref, debit, credit, contra_accounts from(
                                   select  NULL::int as move_id, aml.account_id::int as account_id,0 as case_order, NULL::date as date, aml.analytic_account_id as analytic_id, NULL::text as period_id, NULL::int as journal_id, null::int as  partner_id, null::int as segment_id, null::int as bank_account_id, NULL::int as company_id, ''::varchar as state
                                    , 'INICIAL'::varchar as name, 'INICIAL'::varchar as ref,  case when sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) > 0 then sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) else 0 end AS debit, case when sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) < 0 then sum(COALESCE(aml.credit, 0.0) - COALESCE(aml.debit, 0.0)) else 0 end AS credit,''::text AS contra_accounts
                               from account_move_line aml inner join account_move am on am.id=aml.move_id
                                   where am.date < '%s' %s
                                   group by aml.account_id, aml.analytic_account_id) as bha

                    """ % (context.get('date_from','1900-01-01'), where_bal)

            elif grouping == 'acc_seg':
                balance_union = """UNION
                    select (row_number() over() * -1)::int as id, move_id, account_id, case_order, date, analytic_id, period_id, journal_id, partner_id, segment_id, company_id, state, name, ref, debit, credit, contra_accounts from(
                                   select  NULL::int as move_id, aml.account_id::int as account_id,0 as case_order, NULL::date as date, aml.analytic_account_id as analytic_id, NULL::text as period_id, NULL::int as journal_id, null::int as  partner_id, aml.segment_id, NULL::int as company_id, ''::varchar as state
                                    , 'INICIAL'::varchar as name, 'INICIAL'::varchar as ref,  case when sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) > 0 then sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) else 0 end AS debit, case when sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) < 0 then sum(COALESCE(aml.credit, 0.0) - COALESCE(aml.debit, 0.0)) else 0 end AS credit,''::text AS contra_accounts
                               from account_move_line aml inner join account_move am on am.id=aml.move_id
                                   where am.date < '%s' %s
                                   group by aml.account_id, aml.segment_id) as bha
                ...
                balance_union = """UNION
                    select (row_number() over() * -1)::int as id, move_id, account_id, case_order, date, analytic_id, period_id, journal_id, partner_id, segment_id, bank_account_id, company_id, state, name, ref, debit, credit, contra_accounts from(
                                   select  NULL::int as move_id, aml.account_id::int as account_id,0 as case_order, NULL::date as date, aml.analytic_account_id as analytic_id, NULL::text as period_id, NULL::int as journal_id, null::int as  partner_id, null::int as segment_id, aml.bank_account_id, NULL::int as company_id, ''::varchar as state
                                    , 'INICIAL'::varchar as name, 'INICIAL'::varchar as ref,  case when sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) > 0 then sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) else 0 end AS debit, case when sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) < 0 then sum(COALESCE(aml.credit, 0.0) - COALESCE(aml.debit, 0.0)) else 0 end AS credit,''::text AS contra_accounts
                               from account_move_line aml inner join account_move am on am.id=aml.move_id
                                   where am.date < '%s' %s
                                   group by aml.account_id, aml.bank_account_id) as bha
        ...
        view_query = """
            ...
            CREATE MATERIALIZED VIEW poi_libro_mayor AS (
                select urep.id, urep.move_id, urep.account_id as account_id_a,
                                    ...
                                    case_order, urep.date, urep.partner_id, urep.segment_id, urep.bank_account_id, urep.company_id,
                                    urep.state,
                ...
                left outer join account_journal t4 on t4.id = urep.journal_id

                left outer join (select cr.name::date as from_date,cr.rate
...
class poi_libro_mayor_partner_acc(models.Model):
    ...
    def build_view(self, cr, context=None):
        ...
        if context.get('balance', False):
            order_clause = order_clause.replace(',date',',case_order,date')
            balance_union = """UNION
                           select (row_number() over() * -1)::int as id, move_id, account_id, case_order, date, analytic_id, period_id, journal_id, partner_id, segment_id, bank_account_id, company_id, state, name, ref, debit, credit, contra_accounts 
                           from(
                           select NULL::int as move_id, aml.account_id::int as account_id,0 as case_order, NULL::date as date, NULL::int AS analytic_id, NULL::text as  period_id, NULL::int as journal_id, aml.partner_id as partner_id, null::int as segment_id, NULL::int as bank_account_id, NULL::int as company_id, ''::varchar as state
                                , 'INICIAL'::varchar as name, 'INICIAL'::varchar as ref
                                ...
                                , case when sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)) < 0 then sum(COALESCE(aml.credit, 0.0) - COALESCE(aml.debit, 0.0)) else 0 end AS credit,''::text AS contra_accounts
                           from account_move_line aml inner join account_move am on am.id=aml.move_id
                           where am.date < '%s' %s
                           group by aml.partner_id,aml.account_id) as bha
                            """ % (context.get('date_from','1900-01-01'), where_bal)
