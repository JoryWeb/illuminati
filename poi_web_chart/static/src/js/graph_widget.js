...
openerp.poi_web_chart.Chart = openerp.web.Widget.extend({
    ...
    init: function(parent, model,  domain, options) {
        ...
        this.model = model;
        this.domain = domain;
        this.mode = options.mode || 'line';  // pivot, bar, pie, line
    ...
    start: function() {
        ...
        var indexes = {'pivot': 0, 'line': 1, 'bar': 2 , 'column': 3, 'pie': 4, 'spider': 5};
        ...
        openerp.session.rpc('/poi_web_chart/check_xlwt').then(function (result) {
        ...
        return this.model.call('fields_get', {
            ...
            self.pivot = new openerp.poi_web_chart.PivotTable(self.model, self.domain, self.fields, self.pivot_options);
    ...
    get_search_fields: function () {
        ...
        /*var groupbygroups = _(this.search_view.search_fields).select(function (g) {
    ...
    events: {
        ...
        'click .web_chart_click' : 'header_cell_clicked',
        'click a.field-selection' : 'field_selection2',
    },
    ...
    header_cell_clicked: function (event) {
        ...
        this.dropdown = $(QWeb.render('field_selection2', {fields:fields, header_id:id}));
    ...
    field_selection2: function (event) {
        var id = event.target.getAttribute('data-id'),
    ...
    build_headers: function () {
        ...
        if (pivot.cols.headers != null && pivot.cols.headers.length === 1) {
    ...
    display_data: function () {
        ...
        if (this.pivot.no_data) {
            ...
            if (this.mode === 'pivot') {
                ...
                switch(this.mode) {
                    case 'line':
                        this.$('.graph_main_content').append($('<div class="chart_graph"></div>'));
                        this.draw_common_chart('line', false);
                        break;
                    case 'bar':
                        this.$('.graph_main_content').append($('<div class="chart_graph"></div>'));
                        this.draw_common_chart('bar', false);
                        break;
                    case 'column':
                        this.$('.graph_main_content').append($('<div class="chart_graph"></div>'));
                        this.draw_common_chart('column', false);
                        break;
                    case 'pie':
                        this.$('.graph_main_content').append($('<div class="chart_graph"></div>'));
                        this.draw_pie_chart();
                    ...
                    case 'spider':
                        this.$('.graph_main_content').append($('<div class="chart_graph"></div>'));
                        this.draw_common_chart('line', true);
                        break;
                    default:
    ...
    // Drawing the table
    // ----------------------------------------------------------------------
    draw_common_chart: function (type, polar) {
        ...
        for (var k = 0; k < data.length; k++){
            xaxis.push(data[k].key);
            for (var v = 0; v < data[k].values.length; v++){
                ns_found = false;
                for (var n = 0; n < new_series.length; n++){
                    if (new_series[n].name == data[k].values[v].x) {
                        ns_found = true;
                        new_series[n].data.push(data[k].values[v].y)
                        break;
                    }
                }

                if (!ns_found){
                    new_series.push({name: data[k].values[v].x,
                    data: [data[k].values[v].y]});
                }
            }
            series.push(data[k].values[0].y);

        }


        this.$('.graph_main_content div.chart_graph').highcharts({
            chart: {
                zoomType: 'xy',
                height: self.height,
                type: type,
            ...
            yAxis: {
                title: {
                    text: self.pivot.measures[0].string,
                },
                plotLines: [{
                    value: 0,
                    width: 1,
        ...
        for (var k = 0; k < data.length; k++){
            for (var v = 0; v < data[k].values.length; v++){
                ...
                for (var n = 0; n < new_series.length; n++){
                    if (new_series[n].name == data[k].values[v].x) {
                        ns_found = true;
                        new_series[n].data.push({name: data[k].key, y: data[k].values[v].y})
                        break;
                    }
                }
                ...
                if (!ns_found){
                    new_series.push({name: data[k].values[v].x,
                    data: [{name: data[k].key, y: data[k].values[v].y}]});
                }
            }
            series.push(data[k].values[0].y);
            //data[k].values[v].x
    ...
    make_header_cell: function (header, frozen) {
        ...
        if (_.has(header, 'expanded')) {
            if (('indent' in header) && header.indent >= frozen) {
                ...
                $content.addClass('web_chart_click');
            }
