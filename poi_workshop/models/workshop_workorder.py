...
class WorkshopWorkOrder(models.Model):
    _name = 'workshop.workorder'
    ...
    company_id = fields.Many2one(
        'res.company', 'Company',
        default=lambda self: self.env['res.company']._company_default_get('workshop.workorder'),
        required=True)
    mo_ids = fields.One2many('workshop.order', 'wo_id', 'Maintenance Order')

    @api.model
    def create(self, values):
        if not values.get('name', False) or values['name'] == _('New'):
            values['name'] = self.env['ir.sequence'].next_by_code('workshop.workorder') or _('New')
        return super(WorkshopWorkOrder, self).create(values)
