...
class ProductDateSoldReturnedOrders(models.Model):
    _name = 'product.date.sold.returned.orders'
...
(
	SELECT
	    shop_id,
		cast(cast(date_order as timestamp) + c_interval as date) as date_order, --USING THE NEW c_interval
		product_id,
		sum(pos_qty_sold) as pos_qty_sold,
		sum(amount_sold) as amount_sold,
		sum(amount_discounted) as amount_discounted,
		...
		sum(pos_qty_returned) as pos_qty_returned,
		sum(amount_returned) as amount_returned,
		count(pos_order_id) as num_orders_sold,
		0.0 as num_orders_returned
	FROM product_pos_orders_list
	LEFT JOIN
	(
	...
	ON company_tz.id = product_pos_orders_list.company_id
	WHERE pos_qty_sold > 0 AND pos_qty_returned = 0
	GROUP BY cast(cast(date_order as timestamp) + c_interval as date), product_id, shop_id --USING THE NEW c_interval
	...
	shop_id,
		...
		sum(pos_qty_returned) as pos_qty_returned,
		sum(amount_returned) as amount_returned,
		0.0 as num_orders_sold,
		count(pos_order_id) as num_orders_returned
	FROM product_pos_orders_list
	LEFT JOIN
	(
		SELECT id, cast(substring(tz_offset from 1 for 3)||':'||substring(tz_offset from 4 for 2) as interval) as c_interval FROM res_company
...
GROUP BY date_order, product_id, categ_id, shop_id
ORDER BY date_order, product_id
    ...
    def init(self, cr, select=False, mlc=False, mlr=False):
        table = "product_date_sold_returned_orders"
        cr.execute("""
            SELECT table_type
