...
class report_kardex_dos_wizard_pre(models.TransientModel):
    """
    For Reporte kardex valorado
    """
    _name = "reporte.kardex.dos.wizard.pre"
    ...
    def open_table(self):
        ...
        if data['lot_id']:
            lot_id = str(data['lot_id'][0])

        self.env['poi.report.kardex.valorado.dos.pre'].init(date_from=date_from, date_to=date_to, product_id=product_id, location_id=location_id,lot_id=lot_id)
        context_report = {}
        domain_report = []
        name_context = ""
        ...
        model_data_id = self.env['ir.model.data']._get_id('poi_x_pretensa', 'poi_report_kardex_dos_pre_tree')
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'tree',
            'res_model': 'poi.report.kardex.valorado.dos.pre',
            'view_id': res_id,
    ...
    def _print_report(self, data,):
        ...
        return self.env['report'].get_action(self, 'poi_x_pretensa.report_kardex_pre', data=data)
    ...
    def _print_report_valorado(self, data, ):
        data['form'].update(self.read(['product_id', 'location_id', 'lot_id', 'date_from', 'date_to'])[0])
        return self.env['report'].get_action(self, 'poi_x_pretensa.report_kardex_valorado_pre', data=data)
