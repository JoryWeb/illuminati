...
class stock_return_picking_line(osv.osv_memory):
    _name = "stock.recovered.picking.line"
    _rec_name = 'product_id'

    _columns = {
        'product_id': fields.many2one('product.product', string="Producto", required=True),
        'quantity': fields.float("Cantidad Rec.", digits_compute=dp.get_precision('Product Unit of Measure'), required=True),
        'lot_id': fields.many2one('stock.production.lot', "Serie Recuperado"),
        'wizard_id': fields.many2one('stock.recovered.picking', string="Wizard"),
        'move_id': fields.many2one('stock.move', "Movimiento"),
    }


class stock_return_picking(osv.osv_memory):
    _name = 'stock.recovered.picking'
    _description = 'Return Picking'
    _columns = {
        'product_return_moves': fields.one2many('stock.recovered.picking.line', 'wizard_id', 'Moves'),
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        if pick:
            ...
            for move in pick.move_lines:
                ...
                # No llenar con movimientos actuales este listado servira solo para cargar movimientos nuevos con sus lotes respectivos
                #result1.append((0, 0, {'product_id': move.product_id.id, 'quantity': move.product_uom_qty, 'move_id': move.id}))
    ...
    def _create_recovereds(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        record_id = context and context.get('active_id', False) or False
        ...
        pick_obj = self.pool.get('stock.picking')
        uom_obj = self.pool.get('product.uom')
        data_obj = self.pool.get('stock.recovered.picking.line')
        pick = pick_obj.browse(cr, uid, record_id, context=context)
    ...
    def create_recovered(self, cr, uid, ids, context=None):
        """
         Creates return picking and returns act_window to new picking
        """
        new_picking_id, pick_type_id = self._create_recovereds(cr, uid, ids, context=context)
