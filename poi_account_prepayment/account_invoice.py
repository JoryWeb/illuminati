    ...
    def assign_outstanding_credit(self, cr, uid, id, credit_aml_id, context=None):
        ...
        if not credit_aml.currency_id and inv.currency_id != inv.company_id.currency_id:
            credit_aml.with_context(allow_amount_currency=True).write({
                'amount_currency': inv.company_id.currency_id.with_context(date=credit_aml.date).compute(
                    credit_aml.balance, inv.currency_id),
                'currency_id': inv.currency_id.id})

        if credit_aml.payment_id:
            credit_aml.payment_id.write({'invoice_ids': [(4, id, None)]})

        if credit_aml.payment_id and credit_aml.account_id and credit_aml.account_id.id != inv.account_id.id:
            return inv.register_prepayment(credit_aml)
