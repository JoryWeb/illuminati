...
class AccountReportAnalyticManager(models.TransientModel):
    ...
    _description = 'manages analytic for reports'

    analytic_ids = fields.Many2many('account.analytic.account', relation='account_report_context_analytic')
    available_analytic_ids = fields.Many2many('account.analytic.account', relation='account_report_context_available_analytic', default=lambda s: [(6, 0, s.env['account.analytic.account'].search([]).ids)])

    @api.multi
    def get_available_analytic_ids_and_names(self):
        #TODO!: We do need some custom analytics... I think that it's better to add group by parent too.
        return [[c.id, c.display_name] for c in self.env['account.analytic.account'].search([],order='full_name')]
...
#
# class AccountReportSegmentManager(models.TransientModel):
#     _name = 'account.report.segment.manager'
#     _description = 'manages segments for reports'
#
#     segment_ids = fields.Many2many('account.segment', relation='account_report_context_segment')
#     available_segment_ids = fields.Many2many('account.segment', relation='account_report_context_available_segment', default=lambda s: [(6, 0, s.env['account.segment'].search([]).ids)])
#
#     @api.multi
#     def get_available_segment_ids_and_names(self):
#         #TODO!: We do need some custom analytics... I think that it's better to add group by parent too.
#         return [[c.id, c.display_name] for c in self.env['account.segment'].search([], order='name')]
#
#     @api.model
#     def get_available_segments(self):
#         return self.env['account.segment'].search([])
#
#
# class AccountReportAnalyticTagManager(models.TransientModel):
#     _name = 'account.report.analytic.tag.manager'
#     _description = 'manages analytics for reports'
#
#     analytic_tag_ids = fields.Many2many('account.analytic.tag', relation='account_report_context_analytic_tag')
#     available_analytic_tag_ids = fields.Many2many('account.analytic.tag', relation='account_report_context_available_analytic_tag', default=lambda s: [(6, 0, s.env['account.analytic.tag'].search([]).ids)])
#
#     @api.multi
#     def get_available_analytic_tag_ids_and_names(self):
#         #TODO!: We do need some custom analytics... I think that it's better to add group by parent too.
#         return [[c.id, c.display_name] for c in self.env['account.analytic.tag'].search([], order='name')]
#
#     @api.model
#     def get_available_analytic_tags(self):
...
class AccountReportContextCommon(models.TransientModel):
    ...
    _inherits = {'account.report.footnotes.manager': 'footnotes_manager_id',
                 ...
                 #'account.report.segment.manager': 'segment_manager_id',
                 #'account.report.analytic.tag.manager': 'analytic_tag_manager_id',
                 }
    ...
    all_accounts = fields.Boolean('Use all accounts (not only with movement ones)', default=False, required=True)
    ...
    display_analytic_tags = fields.Boolean("Display Analytic Tag Filter")
    analytic_manager_id = fields.Many2one('account.report.analytic.manager', string='Analytic Accounts Manager', required=True, ondelete='cascade')
    segment_manager_id = fields.Many2one('account.report.segment.manager', string='Segment Accounts Manager', required=True, ondelete='cascade')
    analytic_tag_manager_id = fields.Many2one('account.report.analytic.tag.manager', string='Analytic Tags Manager', required=True, ondelete='cascade')
    ...
    def get_pdf(self):
        ...
        body = self.pool['ir.ui.view'].render(
            self._cr, self._uid, "poi_account_reports.report_financial_letter",
    ...
    def get_html_and_data(self, given_context=None):
        ...
        if given_context:
            ...
            for field in given_context:
                ...
                if self._fields.get(field) and given_context[field] != 'undefined':
                    ...
                    if field == 'company_ids' or field == 'analytic_ids' or field == 'segment_ids' or field == 'analytic_tag_ids': #  Needs to be treated differently as it's a many2many
        ...
        self.display_segments = self.get_report_obj().display_segments
        self.display_analytic = self.get_report_obj().display_analytic
        self.display_analytic_tags = self.get_report_obj().display_analytic_tags
        ...
        select = ['id', 'date_filter', 'date_filter_cmp', 'date_from', 'date_to', 'periods_number', 'date_from_cmp', 'date_to_cmp', 'cash_basis', 'all_entries', 'all_accounts', 'company_ids', 'multi_company', 'display_segments', 'display_analytic', 'display_analytic_tags', 'analytic_ids', 'segment_ids', 'analytic_tag_ids']
        ...
        result['available_analytics'] = self.analytic_manager_id.get_available_analytic_ids_and_names()
        result['available_segments'] = self.segment_manager_id.get_available_segment_ids_and_names()
        result['available_analytic_tags'] = self.analytic_tag_manager_id.get_available_analytic_tag_ids_and_names()
        return result
