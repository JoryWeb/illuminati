odoo.define('poi_account_reports.ReportWidget', function (require) {
'use strict';
...
var ReportWidget = Widget.extend({
    ...
    start: function() {
        QWeb.add_template("/poi_account_reports/static/src/xml/account_report_financial_line.xml");
    ...
    outboundLink: function(e) {
        ...
        if (!_.isUndefined(force_context)) {
            var context = {
                ...
                all_accounts: this.context.all_accounts,
                company_ids: this.context.company_ids,
                analytic_ids: this.context.analytic_ids,
                segment_ids: this.context.segment_ids,
                analytic_tag_ids: this.context.analytic_tag_ids,
            };
                ...
                new Model('account.report.context.common').call('get_full_report_name_by_report_name', [report_name]).then(function (result) {
                    ...
                    var f = function (lines) {// After loading the line
                        self.context_model.call('get_columns_types', [[parseInt(context_id, 10)]]).then(function (types) {
                            var line;
                            lines.shift();
                            for (line in lines) { // Render each line -- AQUI SE RENDERIZA CADA LINEA
