odoo.define('poi_account_reports.account_report_generic', function (require) {
'use strict';
...
var ReportWidget = require('poi_account_reports.ReportWidget');
...
var account_report_generic = Widget.extend(ControlPanelMixin, {
    ...
    get_html: function() {
        ...
        return new Model('account.report.context.common').call('return_context', [self.report_model, self.given_context, self.report_id]).then(function (result) {
            ...
            return self.context_model.call('get_html_and_data', [self.context_id, self.given_context], {context: session.user_context}).then(function (result) {
                ...
                self.report_context.available_analytics = result.available_analytics;
                self.report_context.available_segments = result.available_segments;
                self.report_context.available_analytic_tags = result.available_analytic_tags;
                self.render_buttons();
    ...
    render_searchview_buttons: function() {
        ...
        if (this.report_context.display_analytic) { // Same for th ecompany filter
            this.$searchview_buttons.find('.o_account_reports_analytic-accounts').bind('click', function (event) {
                var report_context = {};
                var value = $(event.target).parents('li').data('value');
                if(self.report_context.analytic_ids.indexOf(value) === -1){
                    report_context.add_analytic_ids = value;
                }
                else {
                    report_context.remove_analytic_ids = value;
                }
                self.restart(report_context);
            });
        }

        if (this.report_context.display_segments) { // Same for th ecompany filter
            this.$searchview_buttons.find('.o_account_reports_segments').bind('click', function (event) {
                var report_context = {};
                var value = $(event.target).parents('li').data('value');
                if(self.report_context.segment_ids.indexOf(value) === -1){
                    report_context.add_segment_ids = value;
                }
                else {
                    report_context.remove_segment_ids = value;
                }
                self.restart(report_context);
            });
        }
        ...
        if (this.report_context.display_analytic_tags) { // Same for th ecompany filter
            this.$searchview_buttons.find('.o_account_reports_analytic_tags').bind('click', function (event) {
                var report_context = {};
                var value = $(event.target).parents('li').data('value');
                if(self.report_context.analytic_tag_ids.indexOf(value) === -1){
                    report_context.add_analytic_tag_ids = value;
                }
                else {
                    report_context.remove_analytic_tag_ids = value;
                }
                self.restart(report_context);
            });
        }
