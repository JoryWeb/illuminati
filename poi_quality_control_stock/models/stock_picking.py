...
from openerp.addons.poi_quality_control.models.qc_trigger_line import\
    ...
    def do_transfer(self):
        ...
        for operation in self.pack_operation_ids:
            if operation.pack_lot_ids:
                for operation_lot in operation.pack_lot_ids:
                    ...
                    for trigger_line in _filter_trigger_lines(trigger_lines):
                        inspection_model._make_inspection(operation_lot, trigger_line)
            else:
                qc_trigger = self.env['qc.trigger'].search(
                    [('picking_type', '=', self.picking_type_id.id)])
                trigger_lines = set()
